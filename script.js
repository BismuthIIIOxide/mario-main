...
var cam;
var scene = "home";
var mario = {}, luigi = {}, obama = {}, yoda = {}, engi = {}, cube = {}, goomba, turtle, bowser, coin, joystick = {}, cloud;
var clouds = [];
...
function setup() {
  ...
  obama.r = loadImage('or.png');
  obama.l = loadImage('ol.png');
  engi.r = loadImage('er.png');
  engi.l = loadImage('el.png');
  yoda.r = loadImage('yr.png');
  yoda.l = loadImage('yl.png');
...
var Player = function (x, y, w, h) {
  ...
  this.update = function (blocks) {
    ...
    if (this.xvel < 0) {
      if (localStorage.getItem("char") == "mario") {
        fill(255, 0, 0);
        image(mario.l, this.x, this.y, this.w, this.h);
      } else if (localStorage.getItem("char") == "luigi") { 
        fill(255, 0, 0);
        image(luigi.l, this.x, this.y, this.w, this.h);
      } else if (localStorage.getItem("char") == "obama") { 
        fill(255, 0, 0);
        image(obama.l, this.x, this.y, this.w, this.h);
      } else if (localStorage.getItem("char") == "yoda") { 
        fill(255, 0, 0);
        image(yoda.l, this.x, this.y, this.w, this.h);
      } else if (localStorage.getItem("char") == "engi") { 
        fill(255, 0, 0);
        image(engi.l, this.x, this.y, this.w, this.h);
      } else if (localStorage.getItem("char") == "cube") { 
        fill(255, 0, 0);
        rect(this.x, this.y, this.w, this.h);
      }
      // fill(255, 0, 0);
      // image(localStorage.getItem("char")=="mario"?mario.l:luigi.l, this.x, this.y, this.w, this.h);
    }
    if (this.xvel >= 0) {
      if (localStorage.getItem("char") == "mario") {
        fill(255, 0, 0);
        ...
        fill(255, 0, 0);
        image(luigi.r, this.x, this.y, this.w, this.h);
      } else if (localStorage.getItem("char") == "obama") { 
        fill(255, 0, 0);
        image(obama.r, this.x, this.y, this.w, this.h);
      } else if (localStorage.getItem("char") == "yoda") { 
        fill(255, 0, 0);
        image(yoda.r, this.x, this.y, this.w, this.h);
      } else if (localStorage.getItem("char") == "engi") { 
        fill(255, 0, 0);
        image(engi.r, this.x, this.y, this.w, this.h);
      } else if (localStorage.getItem("char") == "cube") { 
        fill(255, 255, 0);
        rect(this.x, this.y, this.w, this.h);
      }
      // fill(255, 255, 0);
      // image(localStorage.getItem("char")=="mario"?mario.r:luigi.r, this.x, this.y, this.w, this.h);
    }
  };
  this.draw = function () {
  ...
  this.applyCollision = function (obj, velx, vely) {
    for (var i = 0; i < obj.length; i++) {
      if (collide(this, obj[i]) && obj[i].solid) { // handle collisions
        ...
        if (obj[i].type === 'fsolid' || obj[i].type === 'fbrick') {
          obj[i].fake = true;
        }
        if (obj[i].type ==='elevator') {
          obj[i].elevate = true;
        }
        if (vely > 0) {
          this.yvel = 0;
...
var Block = function (x, y, w, h, type, i) {
  ...
  this.update = function () {
    ...
    if (this.type === 'fsolid' || this.type === 'fbrick') {
      this.solid = false;
    }
    if (this.type === "elevator") {
      ...
      if (this.op < 50) {
        this.solid = false;
        console.log('this.op < 50');
      }
      if (this.op < -120) {
        this.elevate = false;
        this.solid = true;
        this.op = 255;
        console.log('this.op < -120');
      }
    }
  };
};
...
//@invis box
var InvisBox = function (x, y, radius, type, i) {
  this.x = x;
  this.y = y;
  this.w = radius;
  this.h = radius;
  ...
  this.type = type;
  this.draw = function () {
    //invisible code for an invisble block
  };
  this.update = function () {
    if (collide(this, player)) {
      score+=3;
      localStorage.setItem("score",score);
      var i = inboxes.indexOf(this);
      inboxes.splice(i, 1);
      if(this.type == "invis"){  
        blocks.add(this.x, this.y, this.w, this.h, "opened");
        blocks.apply();
...
var inboxes = [];
inboxes.add = function (x, y, r, t) {
  this.push(new InvisBox(x, y, r, t));
};
inboxes.apply = function () {
  for (var i = 0; i < this.length; i++) {
    this[i].draw();
    this[i].update();
  }
...
var ilava = [];
ilava.add = function (x, y, s, t) {
  ilava.push(new Ilava(x, y, s, t));
};
ilava.apply = function () {
  for (var i = 0; i < ilava.length; i++) {
    ilava[i].draw();
    ilava[i].update();
  }
};
...
//@invis lava
var invislava = function (x, y, s, t, i) {
  this.x = x;
  this.y = y;
  this.w = s;
  this.h = s;
...
var inlava = [];
inlava.add = function (x, y, s, t) {
  inlava.push(new invislava(x, y, s, t));
};
inlava.apply = function () {
  for (var i = 0; i < inlava.length; i++) {
    inlava[i].draw();
    inlava[i].update();
  }
};
...
var turtles = [];
turtles.add = function (x, y, w, h) {
  turtles.push(new Turtle(x, y, w, h));
};
turtles.apply = function () {
...
var objects = [blocks, portals, lava, ilava, inlava, spikes, jumpBlocks, cannons, bullets, monsters, turtles, coins, mboxes, inboxes];
...
var updateMap = function () {
  ...
  for (var col = 0; col < levels[level].length; col++) {
    ...
    for (var row = 0; row < cells.length; row++) {
      ...
      break;
      case "δ":
        blocks.add(row * grid, col * grid, grid, grid, "fsolid");
      break;
      case "χ":
        blocks.add(row * grid, col * grid, grid, grid, "fbrick");
      break;
      case "@":
      ...
      break;
      case "e":
        blocks.add(row * grid, col * grid, grid, grid, "elevator");
      break;
      case "X":
      ...
      break;
      case "+":
        ilava.add(row * grid, col * grid, grid, "ilava");
      break;
      case "Z":
        inlava.add(row * grid, col * grid, grid, "inlava");
      break;
      case "?":
      ...
      break;
      case "I":
        inboxes.add(row * grid, col * grid, grid, "invis");
      break;
      case "P":
      ...
      break;
      case "t":
        turtles.add(row * grid, col * grid + grid * 2 / 3, grid, grid / 3);
      break;
      case "O":
...
var draw = function () {
  ...
  switch(scene){
    ...
    text("\n\nDeaths: " + deaths/2, width/2, height/2);
    text("\n\n\n\nPoints: " + floor(score/2), width/2, height/2);
    break;
  }
};
